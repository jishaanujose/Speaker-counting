...
def prediction():
  sig=request.files['img']
  sig.save("sig.wav")
  wav_data, sample_rate1 = sf.read('/content/sig.wav')
  recov=reduce_noise(wav_data)
  ...
  if speak==[0]:
    ...
    predd='More than two'
  return render_template("index.html",data=predd)
